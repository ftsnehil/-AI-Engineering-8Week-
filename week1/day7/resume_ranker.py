...
class jobD(BaseModel):
    ...
    prefered_skill:list[str]
    minimum_experience:float|None     #(none so that some one doesnt have will dont show error)
    educational_requrirement:list[str]
...
your job is to analyze job description and extrtact 
structured information from them.
Return only valid JSON matching this schema 
{job_schema}
IMPORTANT:
Do not return the scghema itself ,
Do NOT return fields like "properties", "title" or "type".
Fill the schema with actual information extracted from the job description.

If minimum experience is not mentioned, return null.
If information for a list is missing, return an empty list.
Do not invent information. # apnea mn se kch v ni dena hai
"""
...
# converting into redable (json) format
...
class Resume(BaseModel):
    ...
    # having none = none or [] so that it doent give error if field is missing in resume                         
...
def final_score(job,resume):
    ...
    return MatchResult(**data)  # return json data


# same what we perform in job description herere also we are converting text (resume) into json formate
...
# read_pdf is a fuction which take the pdf if have many pages and have text then it add / comvbine all texts at one place
...
def read_docx(file_path):
    ...
    text = ""
    for paragraph in document.paragraphs:  # extracting parahraph into text
        if paragraph.text.strip():
            text += paragraph.text + "\n"
    
    for table in document.tables:          # exxtracting table (can be of education )
...
# both function is created now we see the suffix of file nname and extract accordingly
...
all_results=[]   # if i hvae 4 resume then i have 4 results

for file_path in resume_folder.iterdir(): ## itterating 
    ...
    time.sleep(5) # so that if there is N no of resumem it dont call lasrge amont of resume at a time so that sever block n ho
    result = final_score(job, parsed_resume) #llm caLL2 (json of both job description and resume)
     #score and details
    time.sleep(5)  # as their is two llm call
    print("Score:", result.score)
    all_results.append({                     # storing every resume result in list so that we can compare all resume and select best one
    ...
    reverse=True                    # sorting in des order
)
top_2 = all_results[:2]
